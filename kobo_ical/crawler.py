...
class KoboCrawler:
            ...
            try:
                ...
                if response.status_code in [403, 429] or (500 <= response.status_code < 600):
                    if attempt < self.max_retries - 1:
                        ...
                        continue
                    elif self.use_playwright_fallback and response.status_code == 403:
    ...
    def parse_article_date(self, soup: BeautifulSoup, article_url: str) -> Optional[date]:
        ...
        date_selectors = ['time[datetime]', '.date', '.published-date', '[class*="date"]', '[class*="Date"]']
        ...
        if m:
            ...
            return jan1 + timedelta(days=-jan1.weekday() + days_offset)
        return None
    ...
    def parse_weekly_article(self, html: str, article_url: str, year: int, week: int) -> List[BookItem]:
        ...
        if not article_date:
            logger.warning(f"Could not determine date for article {article_url}")
            return books
        ...
        for link_elem in ebook_links:
            ...
            if parent and parent not in elements: elements.append(parent)
                ...
                if not title or len(title) < 2:
                    for sel in ['h2', 'h3', 'h4', 'h5', '[class*="title"]', '[class*="Title"]']:
                        ...
                        if t_elem:
                            ...
                            if t and len(t) > 2: title = t; break
                if not title or len(title) < 2:
                    if link_elem: title = link_elem.get('title') or link_elem.get('aria-label')
                title = self.clean_summary(title)
                book_url = urljoin('https://www.kobo.com', link_elem.get('href')) if link_elem else None
                if title and book_url:
                    days_offset = idx % 7
                    book_date = article_date + timedelta(days=days_offset)
                    books.append(BookItem(title=title.strip(), book_url=book_url, article_url=article_url,
                                          date=book_date, week=week, year=year))
            except Exception as e:
    ...
    def crawl_weekly_books(self, start_year: int = 2025, start_week: int = 1,
        ...
        if end_year is None or end_week is None:
            c_year, c_week, _ = date.today().isocalendar()
            end_year, end_week = c_year, min(c_week, 49)
